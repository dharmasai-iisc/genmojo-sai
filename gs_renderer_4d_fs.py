...
from gaussian_model_4d_fs import GaussianModel, BasicPointCloud
...
class Renderer:
    ...
    def add_object(self, ply_path, name, init_zero_deformation=False):
        ...
        print('adding object', name, ply_path)
    ...
    def precompute_nn(self, obj_name, k=16, dist_thresh=0.05):
        ...
        print(vis_means3D.shape, 'vis_means3D shape')
        ...
        for i, obj_name in enumerate(self.gaussians.keys()):
            ...
            if i == 0:
                ...
                l = [
                    ...
                    # {'params': [self.gaussians[obj_name]._opacity], 'lr': opt.opacity_lr * lr_scale, "name": f"{obj_name}_opacity"},
                    ...
                    {'params': [self.gaussians[obj_name]._feat], 'lr': getattr(opt, 'feature_lr', 1e-5), "name": f"{obj_name}_feat"},
                    {'params': self.gaussians[obj_name].decoder.parameters(), 'lr': getattr(opt, 'feature_lr', 1e-5), "name": f"{obj_name}_feat_dec", 'weight_decay': 0.0},
                ]
                ...
                # self.optimizer.add_param_group({
                #     'params': [self.gaussians[obj_name]._opacity],
                #     'lr': opt.opacity_lr * lr_scale,
                #     "name": f"{obj_name}_opacity"
                # })
                self.optimizer.add_param_group({
    ...
    def prepare_render_all(self, timesteps, specify_obj=None, prev_frame_is_zeroth_frame=False):
        ...
        for obj_name in self.gaussians.keys():
            ...
            # sh_deform_T = shs.unsqueeze(0) + delta_sh_T.reshape([len(timesteps), num_pts, 1, -1])
            ...
            # prev_sh_deform_T = shs.unsqueeze(0) + prev_delta_sh_T.reshape([len(timesteps), num_pts, 1, -1])
            ...
            # shs_T_all[obj_name] = sh_deform_T
            # shs_T_all_prev[obj_name] = prev_sh_deform_T
            shs_delta_T_all[obj_name] = delta_sh_T
            shs_delta_T_all_prev[obj_name] = prev_delta_sh_T

            # Get and cache frame 0 deformations
            # time_0 = torch.tensor(0, device=means3D.device).repeat(means3D.shape[0], 1)
            # time_0 = ((time_0.float() / self.T) - 0.5) * 2 # hack
            # (means3D_deform_0,
            # scales_deform_0,
            # rotations_deform_0,
            # opacity_deform_0) = self.gaussians[obj_name]._deformation(means3D.detach(), scales.detach(), 
            #                                                 rotations.detach(), opacity.detach(),
            #                                                 time_0)
            
            # means3D_0_all[obj_name] = means3D_deform_0
            # opacity_0_all[obj_name] = opacity_deform_0
            # scales_0_all[obj_name] = scales_deform_0
            # rotations_0_all[obj_name] = rotations_deform_0
        ...
        # self.prev_shs_deform_T = shs_T_all_prev
        self.prev_shs_delta_T = shs_delta_T_all_prev

        # self.means3D_deform_0 = means3D_0_all
        # self.scales_deform_0 = scales_0_all
        # self.rotations_deform_0 = rotations_0_all
        # self.opacity_deform_0 = opacity_0_all
    ...
    def render_all(
        ...
        for obj_id, obj_name in enumerate(self.gaussians.keys()):
            ...
            if account_for_global_motion:
                ...
                # TODO: we should probably change this so account_for_global_motion is True by default
                # And this is an additional flag
                # So we only come to this code block during SDS
                # Force recentering
                lower_bounds = self.gaussian_bounds[obj_name]['lower']
                center = self.gaussian_bounds[obj_name]['center']
                upper_bounds = self.gaussian_bounds[obj_name]['upper']
                new_lower_bounds = torch.quantile(means3D_final, q=0.1, dim=0)
                new_center = torch.quantile(means3D_final, q=0.5, dim=0)
                new_upper_bounds = torch.quantile(means3D_final, q=0.9, dim=0)

                # means3D_final = means3D_final + (center - new_center).detach()
                # means3D_final = means3D_final * ((upper_bounds - lower_bounds) / (new_upper_bounds - new_lower_bounds)).detach()
                # scales_final = scales_final * ((upper_bounds - lower_bounds) / (new_upper_bounds - new_lower_bounds)).detach()
            ...
            if cut_gaussians and 'background' != obj_name:
            ...
            if 'background' == obj_name and virtual2real_t0 is not None:

                means3D_homo = torch.cat([means3D_masked, torch.ones_like(means3D_masked[:, :1])], dim=-1)
                means3D_warped = (virtual2real_t0.to(means3D_homo.device) @ means3D_homo.T).T
                means3D_masked = means3D_warped[:, :3] / means3D_warped[:, -1:]

                rot_quat = torch.from_numpy(R.from_matrix(virtual2real_t0[:3, :3].numpy()).as_quat()).unsqueeze(0).to(rotations_masked.device)
                rot_quat = rot_quat[:, [3, 0, 1, 2]]
                rotations_from_quats = quat_mult(rot_quat, rotations_masked).float()
                rotations_masked = rotations_from_quats / torch.linalg.norm(rotations_from_quats, dim=-1, keepdims=True)
                ...
                rot_quat = torch.from_numpy(R.from_matrix(virtual2real[:3, :3].numpy()).as_quat()).unsqueeze(0).to(rotations_masked.device)
                rot_quat = rot_quat[:, [3, 0, 1, 2]]
                rotations_from_quats = quat_mult(rot_quat, rotations_masked).float()
                rotations_masked = rotations_from_quats / torch.linalg.norm(rotations_from_quats, dim=-1, keepdims=True)
        ...
        # print('extra_attrs shape', extra_attrs.shape)
        orig_extra_dim = extra_attrs.shape[1]
        extra_feat = torch.cat(all_feats, dim=0).float()
        # print('extra_feat shape', extra_feat.shape)
        extra_attrs = torch.cat([extra_attrs, extra_feat], dim=1)
        ...
        upscaled_extra = self.gaussians[0].decode_feat_map(extra[orig_extra_dim:])
    ...
    def render_flow_all(
        ...
        gaussian_2d_pos_curr, _, gaussian_3d_pos_curr = self.get_2d_gaussian_pos_all(viewpoint_cameras[0], default_camera_center, account_for_global_motion)
        gaussian_2d_pos_prev, _, gaussian_3d_pos_prev = self.get_2d_gaussian_pos_all(viewpoint_cameras[1], default_camera_center, account_for_global_motion)
        ...
        for obj_name in self.gaussians.keys():
            ...
            if account_for_global_motion:
                scales_final = scales_final * self.gaussian_scales[obj_name][viewpoint_cameras[1].time]
                means3D_final = means3D_final * self.gaussian_scales[obj_name][viewpoint_cameras[1].time]
                means3D_final = means3D_final + self.gaussian_translations[obj_name][viewpoint_cameras[1].time]

                # Scale Gaussians
                # Hack to deal with z conventions
                cam_center = default_camera_center.clone()
                cam_center = -cam_center
                means3D_final = cam_center - (cam_center - means3D_final) * self.gaussian_depth_scaling[obj_name][viewpoint_cameras[1].time]
                scales_final = scales_final * self.gaussian_depth_scaling[obj_name][0]
    ...
    def render_3d_pos_all(
        ...
        for obj_name in self.gaussians.keys():
            ...
            # First go from obj centric frame to world frame
            if account_for_global_motion:
                cur_means3D_final = cur_means3D_final * self.gaussian_scales[obj_name][viewpoint_cameras[0].time]
                cur_means3D_final = cur_means3D_final + self.gaussian_translations[obj_name][viewpoint_cameras[0].time]

                # Scale Gaussians
                # Hack to deal with z conventions
                cam_center = default_camera_center.clone()
                cam_center = -cam_center
                cur_means3D_final = cam_center - (cam_center - cur_means3D_final) * self.gaussian_depth_scaling[obj_name][viewpoint_cameras[0].time]
    ...
    def get_2d_gaussian_pos_for_traj(self,
        ...
        for obj_name in self.gaussians.keys():
            ...
            cam_center = default_camera_center.clone()
            cam_center = -cam_center
            means3D_final = cam_center - (cam_center - means3D_final) * self.gaussian_depth_scaling[obj_name][viewpoint_camera.time].detach()
